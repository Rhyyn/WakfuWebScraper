...
class ItemDataSpider(scrapy.Spider):
    ...
    def print_estimated_time(self, ids, scrap_per_min):
        ...
        if math.ceil(len(ids) / scrap_per_min) > 100:
            ...
            print(
                "Estimated time of scrapping : ",
                math.ceil(len(ids) / scrap_per_min),
                "mn",
            )
    ...
    def parse(self, response):
        if response.status == 302 and "Location" in response.headers:
            ...
            self.logger.warning(
                "Redirected to %s. Handling redirection.", redirected_url
            )
        ...
        if response.status == 200:
            ...
            for monster_html in monster_divs.extract():
                ...
                monster_name = (
                    monster_selector.css(".ak-title span::text").get().strip()
                )  # type: ignore
                self.logger.info("---monster_name : %s", monster_name)

                monster_id_url = monster_selector.css(".ak-title a::attr(href)").get()
                monster_id_match = (
                    re.search(r"/(\d+)-", monster_id_url) if monster_id_url else None
                )
                ...
                # Extract drop rate
                drop_rate = (
                    monster_selector.css(".ak-aside::text").get().strip()
                )  # type: ignore
    ...
    def closed(self, reason):
        ...
        file_path = os.path.join(
            ...
            "ScrapedFiles",
            "ScrapedItems",
            f"{current_category_name}_scraped_data.json",
        )
