...
class TestScrapSpider(scrapy.Spider):
    """
    -INFO-
    Test Spider, used for debug/demo.
    -------
    -USAGE-
    ...
    def __init__(self, category_id: int, wanted_amount:int, *args, **kwargs):
        # param gets converted to str for some reason
        # need to convert it back to int | :)
        ...
        super(TestScrapSpider, self).__init__(*args, **kwargs)
    ...
    def closed(self, reason):
        ...
        file_path = os.path.join(os.path.dirname(os.path.realpath(
            __file__)), '..', 'ScrappedData', 'ScrappedFiles', 'ScrapTests', f'test_scrapped_data.json')
