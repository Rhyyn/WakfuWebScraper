...
class MonstersDataSpider(scrapy.Spider):
    ...
    def __init__(self, *args, **kwargs):
        ...
        scraped_monster_dir = os.path.join(project_dir, 'ScrapyProject', 'ScrapedData', 'scrapedFiles', 'scrapedMonsters')
        self.monster_IDs_path = os.path.join(scraped_monster_dir, 'monsters_IDs.json')
        monsters_scraped_data_path = os.path.join(scraped_monster_dir, 'en_monsters_stats_data.json')
    ...
    def parse(self, response):
        if response.status == 200:
            ...
            # monster_level
            monster_level = response.css(
                'div.ak-encyclo-detail-level::text').get()
            ...
            for ak_list_element in charas_container_div.css('div.ak-list-element'):
                ...
                value_span = ak_list_element.css(
                    'div.ak-title span::text').get()

                # Check if both title_div and value_span are not None
                if title_div and value_span:
                    monster_charas.append(
                        {title_div.strip(): value_span.strip()})
            ...
            if drops_panel_content:
                ...
                items_drop_divs = drops_panel_content.css(
                ...
                for item__drop_divs_html in items_drop_divs.extract():
                    ...
                    image_url = item_selector.css(
                        'div.ak-content div.ak-title a::attr(href)').get()
                    self.logger.info("---image_url : %s", image_url)
                    item_drop_name = item_selector.css(
                        'div.ak-content div.ak-title span.ak-linker::text').get()
                    item_drop_rarity = rarity_number
                    item_drop_chance = item_selector.css(
                        'div.ak-text div.ak-drop-percent span:nth-child(2)::text').get()
                    item_drop_level = item_numeric_level
                    item_id_match = re.search(
                        r'/(\d+)-', image_url) if image_url else None
            ...
            self.logger.info("is_monster_capturable: %s",
                             is_monster_capturable)
    ...
    def closed(self, reason):
        ...
        file_path = os.path.join(os.path.dirname(os.path.realpath(__file__)), '..', 'ScrapedData', 'scrapedFiles', 'scrapedMonsters', 'en_monsters_stats_data.json')
